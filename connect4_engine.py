...
class GameUI:
    ...
    def load_model(self, filepath):
        ...
        if AgentClass:
            try:
                self.agent = AgentClass(epsilon=0.0)
            except TypeError:
            ...
            if hasattr(self.agent, 'load'):
                ...
                print(f"Error: Agent in {self.model_name} is missing a 'load' method.")
                self.agent = None
    ...
    def process_move(self, col, piece, name, color):
        if self.env.is_valid_location(col):
            ...
            # Draw the board first so the new piece appears
            self.draw_board()
            
            # Print the winning text on top if the game ends
    ...
    def play(self):
        ...
        while not self.env.game_over:
            ...
            if is_ai_turn and not self.env.game_over:
                ...
                if self.agent:
                    col = self.agent.get_action(self.env.board.copy(), valid_moves, 2)
                else:
